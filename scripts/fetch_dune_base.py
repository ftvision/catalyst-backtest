...
def main() -> int:
    ...
    client = DuneClient(KEY, http_transport(), poll_interval=3.0, max_polls=120)

    gas_id = upsert_query("catalyst: base L1 gas (hourly)", BASE_GAS_SQL, os.environ.get("BASE_GAS_ID"))
    px_id = upsert_query("catalyst: base DEX eth price (hourly)", BASE_DEX_SQL, os.environ.get("BASE_PX_ID"))

    print(f"running base gas {args.start:%Y-%m-%d}..{args.end:%Y-%m-%d} ...")
    ...
    n_px = ingest_candles(
        store, client, venue="base", symbol="ETH", interval="1h",
        query_id=px_id, start=args.start, end=args.end,
    )
    store.set_provenance("candles", "base/ETH", "native")
    print(f"  wrote {n_px} candles -> venue=base/symbol=ETH [native]")
    return 0
