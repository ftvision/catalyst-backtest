...
async fn strategy_repository_is_exposed() {
    ...
    assert!(items
        .iter()
        .any(|item| item["id"] == "g15_evm_yield_add_withdraw"));

    let (s, strategy) = send(&st, "GET", "/strategies/g15_evm_yield_add_withdraw", None).await;
    assert_eq!(s, StatusCode::OK);
    assert_eq!(strategy["id"], "g15_evm_yield_add_withdraw");
    assert_eq!(strategy["graph"]["nodes"].as_array().unwrap().len(), 5);
