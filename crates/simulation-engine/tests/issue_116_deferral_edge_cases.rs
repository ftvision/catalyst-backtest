...
//! - `market_then_market_both_resolve_at_fill_bar_no_double_defer`: a market action
//!   chained after another resolves at the same fill bar (no second-bar deferral).
...
// ---------------------------------------------------------------------------
// (2) market -> market: downstream does NOT defer a second bar.
// ---------------------------------------------------------------------------
...
#[test]
fn market_then_market_both_resolve_at_fill_bar_no_double_defer() {
    // An initial market buy (USDC->ETH) at bar 0, with a downstream market sell
    // (ETH->USDC) wired after it. The upstream market defers to bar 1's open;
    // when it books on bar 1, the downstream market runs at the SAME fill bar and
    // resolves at bar 1's open too — it does NOT defer a second bar to bar 2.
    let g = graph(json!({
        "nodes": [
    ...
    // Both market swaps execute, and BOTH are booked on bar 1 (the same fill bar).
    ...
    for e in &execs {
        assert_eq!(
            e.ts,
            at(1),
            "#116 chain: a downstream market action must resolve at the SAME fill bar (1h) \
             as the upstream — it must NOT defer a second bar to 2h; got {} for node {:?}",
            e.ts, e.node_id
        );
    }
